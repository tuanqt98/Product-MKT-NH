...
export default function ContentSkillsPage() {
  ...
  useEffect(() => {
    fetch('/api/skills')
      .then(res => res.json())
      .then(data => {
        // Lọc các skill thuộc category 'content'
        const contentSkills = data.filter((s: any) => s.category === 'content');
        setSkills(contentSkills);
        setLoading(false);
      })
      .catch(err => {
        console.error(err);
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <header>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-3xl font-bold">Kỹ năng Nội dung</h2>
        </div>
        <p className="text-muted-foreground">Tạo nội dung marketing thu hút, kịch bản và lịch biên tập cho Nhật Hàn.</p>
      </header>
      ...
      <div className="relative max-w-md">
        ...
        <input 
          type="text" 
          placeholder="Tìm kiếm kỹ năng viết bài..." 
