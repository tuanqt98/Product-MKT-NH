...
import { Megaphone, ArrowRight, Sparkles, Search } from 'lucide-react';

export default function AdsSkillsPage() {
  ...
  useEffect(() => {
    fetch('/api/skills')
      .then(res => res.json())
      .then(data => {
        // Lọc các skill liên quan đến quảng cáo/ads/brief chiến dịch
        const adsSkills = data.filter((s: any) => 
          s.id.includes('ads') || 
          s.id.includes('quang-cao') || 
          s.name.toLowerCase().includes('quảng cáo') ||
          s.name.toLowerCase().includes('ads') ||
          s.name.toLowerCase().includes('brief')
        );
        setSkills(adsSkills);
        setLoading(false);
      })
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <header>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-3xl font-bold">Quảng cáo & Chiến dịch</h2>
        </div>
        <p className="text-muted-foreground">Tối ưu hóa các chiến dịch Facebook Ads, TikTok Ads cho Nhật Hàn.</p>
      </header>
      ...
      <div className="relative max-w-md">
        ...
        <input 
          type="text" 
          placeholder="Tìm kỹ năng quảng cáo..." 
